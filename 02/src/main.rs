...
fn find_answer(password_rules: &Vec<PasswordRule>) -> i32 {
    password_rules
    .iter()
    .fold(
        0,
        |acc, password_rule| acc + (if password_rule.is_valid() { 1 } else { 0 })
    )
}
...
fn find_answer_2(password_rules: &Vec<PasswordRule>) -> i32 {
    password_rules
    .iter()
    .fold(
        0,
        |acc, password_rule| acc + (if password_rule.is_valid_2() { 1 } else { 0 })
    )
}
...
impl PasswordRule {
    fn new(line: &str) -> Result<PasswordRule, &'static str> {
        ...
        if trimmed_line.len() == 0 {
            return Err("An empty line cannot be a password rule.");
        }

        let parts: Vec<&str> = trimmed_line.split(&['-', ' ', ':'][..]).filter(|s| !s.is_empty()).collect();
        let min = parts[0].parse::<u32>().unwrap();
        let max = parts[1].parse::<u32>().unwrap();
        let char = parts[2].to_owned().chars().next().unwrap();
    ...
    pub fn is_valid_2(&self) -> bool {
        let a = usize::try_from(self.min - 1).unwrap();
        let b = usize::try_from(self.max - 1).unwrap();

        let c = self.char;
        let password_chars: Vec<char> = self.password.chars().collect();
        
        (password_chars[a] == c && password_chars[b] != c) || (password_chars[a] != c && password_chars[b] == c) 
    }
}
...
fn main() {
    ...
    let password_rules: Vec<PasswordRule> = "
    ...
    "
    .lines()
    .map(PasswordRule::new)
    .filter_map(Result::ok)
    .collect();

    println!("The answer to part 1 is: {:?}", find_answer(&password_rules));
    println!("The answer to part 2 is: {:?}", find_answer_2(&password_rules));
}
